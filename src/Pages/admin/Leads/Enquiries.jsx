        ...
        <table className='table text-sm table-auto w-full'>
            ...
            <tbody className='bg-white divide-y divide-gray-200'>
            {
                [...enquiries].reverse().map((enquiry, index) => (
                    <tr key={index}>
                                      <td className='px-6 py-3 text-left '>{new Date(enquiry.createdAt).toLocaleDateString("en-US", { year: 'numeric', month: 'short', day: 'numeric' })}</td>

                        <td className='px-6 py-4 whitespace-nowrap'>{enquiry.name}</td>
                        <td className='px-6 py-4 whitespace-nowrap'>{enquiry.phone}</td>
                        <td className='px-6 py-4 whitespace-nowrap'>{enquiry.category ? enquiry.category.name : 'N/A'}</td>
                        <td className='px-6 py-4 whitespace-nowrap'>{enquiry.business ? enquiry.business.name : 'N/A'}</td>
