      ...
      >
        ...
        >
          <div className="flex flex-col gap-7 w-full">
            ...
            <div
              className={` flex flex-col ${isSidebarCollapsed ? "items-center" : "items-start"} gap-4 w-full`}
            >
              {sidebarLinks.map(({ name, icon, path }, index) => (
                <Link
                  to={path}
                  key={index}
                  className={`w-full rounded-md p-2 px-4 ${isSidebarCollapsed ? "px-0" : ""} ${isActiveLink(path) ? "bg-blue-500" : ""}`}
                >
                  <div
                    className={`flex items-center ${isSidebarCollapsed ? "justify-center" : "justify-start"} w-full} cursor-pointer gap-3 ${
            ...
            <div className="flex flex-col gap-4">
              {sidebarLinks.map(({ name, icon, path }, index) => (
                <Link
                  to={path}
                  key={index}
                  className={`w-full rounded-md p-2 px-4 ${isActiveLink(path) ? "bg-blue-500 text-white" : ""}`}
                  onClick={() => setDashboardMenuOpen(!dashboardMenuOpen)}
                >
