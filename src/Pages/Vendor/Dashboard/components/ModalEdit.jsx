              ...
              <div className="mt-8 text-center w-full flex gap-4 justify-start">
                ...
                <div className="flex flex-[8] flex-col gap-2 items-start w-full">
                  <div className="w-full flex items-center justify-between gap-2">
                    ...
                    {
                      isEditing ? 
                      <FiEdit2 className="cursor-pointer h-5 w-5 text-gray-600" onClick={() => setIsEditing(!isEditing)} /> 
                      :
                      <FiX className="cursor-pointer h-5 w-5 text-red-500" onClick={() => setIsEditing(!isEditing)} />
                    }
                  </div>
                  <textarea disabled={isEditing} type="text" value={updatedPost.description} onChange={e => setUpdatedPost({ ...updatedPost, description: e.target.value })} className="appearance-none rounded-md relative block w-full px-3 py-2 border h-32 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" />
                  <span>Created at {
