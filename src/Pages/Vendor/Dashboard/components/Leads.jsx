...
const Leads = ({businesses}) => {
  ...
  const handleFilterCallLeads = (filter) => {
    setAllCallLeads(businesses.flatMap((business) => business.callLeads.filter((lead) => {
      ...
      if(filter.value === "yesterday"){
        return new Date(lead.createdAt).toDateString() === new Date(new Date().setDate(new Date().getDate() - 1)).toDateString()
      }
      if(filter.value === "last 7 days"){
        return new Date(lead.createdAt).getDate() >= (new Date().getDate())-7
      }
      if(filter.value === "last 30 days"){
        return new Date(lead.createdAt).getMonth() === new Date().getMonth()
      }
  ...
  const handleFilterEnquiries = (filter) => {
    setAllEnquiries(businesses.flatMap((business) => business.enquiries.filter((enquiry) => {
      if(filter.value === "all"){
        return true
      }
      if(filter.value === "today"){
        return new Date(enquiry.createdAt).toDateString() === new Date().toDateString()
      }
      if(filter.value === "yesterday"){
        return new Date(enquiry.createdAt).toDateString() === new Date(new Date().setDate(new Date().getDate() - 1)).toDateString()
      }
      if(filter.value === "last 7 days"){
        return new Date(enquiry.createdAt).getDate() >= (new Date().getDate())-7
      }
      if(filter.value === "last 30 days"){
        return new Date(enquiry.createdAt).getMonth() === new Date().getMonth()
      }
      if(filter.value === "this year"){
...
<div className='mt-6 flex flex-col gap-5'>
   <div className=''>
    <div className='flex gap-5'>
      ...
      <input type="radio" name="call lead filter" id={`${filter.value}`} onChange={() => handleFilterCallLeads(filter)} value={filter.value}/>
...
<div className='mt-6 flex flex-col gap-5'>
  <div>
    <div className='flex gap-5'>
      ...
      <div className='flex items-center gap-3'>
      <input type="radio" name="enquiry filter" id={`${filter.value}`} onChange={() => handleFilterEnquiries(filter)} value={filter.value}/>
      <label htmlFor={filter.value}>{filter.label}</label>
      </div>
      ))
}
