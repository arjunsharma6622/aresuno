...
const MapComponent = ({ businessDetails, setBusinessDetails }) => {
  ...
  useEffect(() => {
    ...
    const map = new google.maps.Map(mapRef.current, {
      center: { lat: parseFloat(initialCoordinates[1]), lng: parseFloat(initialCoordinates[0]) },
      zoom: 16,
    });

    const newMarker = new google.maps.Marker({
      position: { lat: parseFloat(initialCoordinates[1]), lng: parseFloat(initialCoordinates[0]) },
      map: map,
      draggable: true,
    });
