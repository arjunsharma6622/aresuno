    ...
    <div className="bg-white mb-10">
      <div className="max-w-4xl mx-auto text-center">
        ...
        <p className="text-gray-600 mb-16">
          Our packages are designed to help you grow your business and achieve
          your goals.
        </p>
      </div>
          ...
          <thead className="">
            <tr className="border-r border-red-600">
              ...
              {packages.map((pkg) => (
                <th
                  key={pkg.name}
                  className={`h-[120px] mx-10  relative py-4 px-6  ${pkg.name === "Advance" ? "border-t border-r border-l border-purple-500" : ""} bg-${pkg.name.toLowerCase()}-200`}
                >
                  {pkg.name === "Advance" && (
                  ...
                  <div className="absolute flex items-center gap-1 justify-center flex-col top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 z-[2]">
                    ...
                    <div
                      className={`text-3xl border-${getColor(pkg.name.toLowerCase())} flex items-center gap-0 justify-center font-semibold`}
                    >
                      <FaIndianRupeeSign className="w-5 h-5" />
                      <span>{pkg.price}/yr</span>
                    </div>
                  </div>
          ...
          <tbody>
            {features.map((feature, index) => (
              <tr
                ...
                className={
                  index % 2 === 0
                    ? " border-b border-black border-dashed border-spacing-5 text-center"
                    : "bg-gray-50 text-center border-b border-dashed border-black"
                }
              >
                ...
                {packages.map((pkg) => (
                  <td
                    key={pkg.name}
                    className={`py-4 px-6 text-center ${pkg.name === "Advance" ? "border-r border-l border-purple-500" : ""} `}
                  >
